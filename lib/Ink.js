...
(function () {
  ...
  // , fill   : Color "#000" "rgba(0, 0, 0, 1)"
  // , stroke : Color "#000" "rgba(0, 0, 0, 1)"
  // }
  function Rect(s) {
    ...
    this.fill   = s.fill   || "rgba(0, 0, 0, 0.2)"
    this.stroke = s.stroke || "rgba(0, 0, 0, 0.2)"
    this.rotation = 0
    this.speed    = { x:  0, y:  0 }
  ...
  function Circ(s) {
    ...
    this.y = s.y || 0
    this.r = s.r || 0
    this.fill   = s.fill || "rgba(255, 255, 255, 0.3)"
    this.stroke = s.stroke || "rgba(255, 255, 255, 1)"
    this.rotation = 0
    this.speed    = { x:  0, y:  0 }
    this.maxSpeed = { x: 10, y: 10 }
    this.infiniteScope = null
  ...
  //   x : X position Number
  // , y : Y position Number
  // , v : Array of Vectors composing the path
  // , fill   : Color "#000" "rgba(0, 0, 0, 1)"
  // , stroke : Color "#000" "rgba(0, 0, 0, 1)"
  // }
  function Path(s) {
  ...
  function init(s) {
    ...
    var w       = getWindowSize()
    can.width   = s.width  || w.w // Unprecise way to avoid X overflow
    can.height  = s.height || w.h // Unprecise way to avoid Y overflow
    ...
    if (clear) Ink.draw(new Ink.Rect({ width: can.width, height: can.height, fill: clear }), 1)
