...
~function (W, D, M, U) {
  ...
  function Rect(a) {
    ...
    if (s.border_radius) {
      ...
      path_op = [
          s.x + s.border_radius
        , s.x + s.width - s.border_radius
        , s.x + s.width
        , s.y + s.border_radius
        , s.y + s.height - s.border_radius
        , s.y + s.height
        ]
      s.path        = [
        [ path_op[0], s.y] // Starting point
      , [ path_op[1], s.y, path_op[2], s.y, path_op[2], path_op[3]]
      , [ path_op[2], path_op[4], path_op[2], path_op[5], path_op[1], path_op[5]]
      , [ path_op[0], path_op[5], s.x, path_op[5], s.x, path_op[4]]
      , [ s.x, path_op[3], s.x, s.y, path_op[0], s.y]
      ]
      delete path_op
    }
    s.rotation      = 0
  ...
  function grad(a) {
    ...
    switch (a.type) {
      case 'LINEAR':
        grad = con.createLinearGradient(a.x1, a.y1, a.width, a.height)
        break
      case 'RADIAL':
